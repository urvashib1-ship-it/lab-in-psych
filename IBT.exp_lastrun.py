...
expName = 'IBT.exp'  # from the Builder filename that created this script
expVersion = ''
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/urvashibhansali/Documents/AU [Psych] (1-4 years) /3rd Year/Semester 5 Monsoon 25-26/PSY310/Psychopy/Youtube Psychopy Tutorials/Intentional Binding Task (7)/IBT.exp_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    polygon_2 = visual.ShapeStim(
        ...
        ori=0.0, pos=(0, 0), draggable=False, anchor='center',
        lineWidth=1.0,
        colorSpace='rgb', lineColor=(0.0000, -0.9843, -0.9922), fillColor=(0.0000, -0.9843, -0.9922),
    ...
    trials = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('/Users/urvashibhansali/Documents/AU [Psych] (1-4 years) /3rd Year/Semester 5 Monsoon 25-26/PSY310/Lab Reports/Intentional Binding Task /Conditions File/delay_freq_condition.xlsx'), 
        seed=None, 
        isTrials=True, 
    )
    ...
    for thisTrial in trials:
        ...
        while continueRoutine:
            ...
            # has the Routine been forcibly ended?
            if beep.forceEnded or routineForceEnded:
                break
            # has every Component finished?
            continueRoutine = False
            for thisComponent in beep.components:
        ...
        # create an object to store info about Routine response_routine
        response_routine = data.Routine(
            name='response_routine',
            components=[text, textbox, button],
        )
        response_routine.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        textbox.reset()
        # reset button to account for continued clicks & clear times on/off
        button.reset()
        # store start times for response_routine
        response_routine.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        response_routine.tStart = globalClock.getTime(format='float')
        response_routine.status = STARTED
        thisExp.addData('response_routine.started', response_routine.tStart)
        response_routine.maxDuration = None
        # keep track of which components have finished
        response_routineComponents = response_routine.components
        for thisComponent in response_routine.components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        thisExp.currentRoutine = response_routine
        response_routine.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine:
            # if trial has changed, end Routine now
            if hasattr(thisTrial, 'status') and thisTrial.status == STOPPING:
                continueRoutine = False
            ...
            if thisExp.status == PAUSED:
                pauseExperiment(
                    ...
                    win=win, 
                    timers=[routineTimer, globalClock], 
                    currentRoutine=response_routine,
                )
                # skip the frame we paused on
                continue
            ...
            # has a Component requested the Routine to end?
            if not continueRoutine:
                response_routine.forceEnded = routineForceEnded = True
            # has the Routine been forcibly ended?
            if response_routine.forceEnded or routineForceEnded:
                break
            # has every Component finished?
            continueRoutine = False
            for thisComponent in response_routine.components:
        ...
        for thisComponent in response_routine.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for response_routine
        response_routine.tStop = globalClock.getTime(format='float')
        response_routine.tStopRefresh = tThisFlipGlobal
        thisExp.addData('response_routine.stopped', response_routine.tStop)
    ...
    # completed 55.0 repeats of 'trials'
