...
expName = '2nd exp'  # from the Builder filename that created this script
expVersion = ''
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/urvashibhansali/Documents/AU [Psych] (1-4 years) /3rd Year/Semester 5 Monsoon 25-26/PSY310/Solo Project/2nd exp_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    text_2 = visual.TextStim(win=win, name='text_2',
        text='Welcome!\n\nYou will see short sentences on the screen.\nPress the RIGHT arrow if the sentence is GRAMMATICAL.\nPress the LEFT arrow if it is UNGRAMMATICAL.\n\nTry to respond as quickly and accurately as possible.\n\nPress SPACE to begin.',
        font='Arial',
    ...
    fix_text = visual.TextStim(win=win, name='fix_text',
        ...
        languageStyle='LTR',
        depth=-1.0);
    blank1_bg = visual.Rect(
        win=win, name='blank1_bg',
    ...
    prime_text = visual.TextStim(win=win, name='prime_text',
        ...
        languageStyle='LTR',
        depth=-3.0);
    blank2_bg = visual.Rect(
        win=win, name='blank2_bg',
        width=(2, 2)[0], height=(2, 2)[1],
        ori=0.0, pos=(0, 0), draggable=False, anchor='center',
        lineWidth=1.0,
        colorSpace='rgb', lineColor='white', fillColor='white',
    ...
    text_3 = visual.TextStim(win=win, name='text_3',
        text='thank you for partcipating. ',
        font='Arial',
        pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    trials = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('Conditions file/stimuli_correctAnss.csv'), 
        seed=None, 
        isTrials=True, 
    )
    ...
    for thisTrial in trials:
        ...
        trial = data.Routine(
            name='trial',
            components=[fix_text, blank1_bg, prime_text, blank2_bg, target_text, key_resp],
        )
        ...
        while continueRoutine:
            ...
            if fix_text.status == STARTED:
                ...
                if tThisFlipGlobal > fix_text.tStartRefresh + 0.25-frameTolerance:
            ...
            # if blank1_bg is starting this frame...
            if blank1_bg.status == NOT_STARTED and tThisFlip >= 0.25-frameTolerance:
                # keep track of start time/frame for later
                blank1_bg.frameNStart = frameN  # exact frame index
                blank1_bg.tStart = t  # local t and not account for scr refresh
                blank1_bg.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(blank1_bg, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'blank1_bg.started')
                # update status
                blank1_bg.status = STARTED
                blank1_bg.setAutoDraw(True)
            
            # if blank1_bg is active this frame...
            if blank1_bg.status == STARTED:
                # update params
                pass
            
            # if blank1_bg is stopping this frame...
            if blank1_bg.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > blank1_bg.tStartRefresh + 0.2-frameTolerance:
                    # keep track of stop time/frame for later
                    blank1_bg.tStop = t  # not accounting for scr refresh
                    blank1_bg.tStopRefresh = tThisFlipGlobal  # on global time
                    blank1_bg.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'blank1_bg.stopped')
                    # update status
                    blank1_bg.status = FINISHED
                    blank1_bg.setAutoDraw(False)
            
            # *prime_text* updates
            
            # if prime_text is starting this frame...
            if prime_text.status == NOT_STARTED and tThisFlip >= 0.45-frameTolerance:
                # keep track of start time/frame for later
            ...
            if prime_text.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > prime_text.tStartRefresh + 0.2-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            # if blank2_bg is starting this frame...
            if blank2_bg.status == NOT_STARTED and tThisFlip >= 0.65-frameTolerance:
                # keep track of start time/frame for later
                blank2_bg.frameNStart = frameN  # exact frame index
                blank2_bg.tStart = t  # local t and not account for scr refresh
                blank2_bg.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(blank2_bg, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'blank2_bg.started')
                # update status
                blank2_bg.status = STARTED
                blank2_bg.setAutoDraw(True)
            
            # if blank2_bg is active this frame...
            if blank2_bg.status == STARTED:
                # update params
                pass
            
            # if blank2_bg is stopping this frame...
            if blank2_bg.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > blank2_bg.tStartRefresh + 0.1-frameTolerance:
                    # keep track of stop time/frame for later
                    blank2_bg.tStop = t  # not accounting for scr refresh
                    blank2_bg.tStopRefresh = tThisFlipGlobal  # on global time
                    blank2_bg.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'blank2_bg.stopped')
                    # update status
                    blank2_bg.status = FINISHED
                    blank2_bg.setAutoDraw(False)
            
            # *target_text* updates
            
            # if target_text is starting this frame...
            if target_text.status == NOT_STARTED and tThisFlip >= 0.75-frameTolerance:
                # keep track of start time/frame for later
            ...
            if key_resp.status == NOT_STARTED and tThisFlip >= 0.75-frameTolerance:
                # keep track of start time/frame for later
        ...
        # Check if this is the end of a 60-trial block
        if (trials.thisTrialN + 1) % 60 == 0:
            # Pause after every 60th trial
            continueRoutine = False
            # Show the pause screen
            thisExp.nextEntry()  # end the current trial's data entry
            pauseLoop = True
        else:
            pauseLoop = False
    ...
    # completed 23.0 repeats of 'trials'
